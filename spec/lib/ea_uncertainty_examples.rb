module EaUncertaintyExamples
  module ClassMethods
    def load_examples
      ...
      #
      # NOTE:
      # k, uexp of this example have been corrected, as EA/4-02 assumes k=2, which does not hold when checking
  ...
  module InstanceMethods
    RSpec::Matchers.define :comply_with_example do |ex_num|
      match do |actual|
        ...
        actual = data.compute
        ((actual.uy - data.uy).abs <= 0.01*data.uy) &&
        ((actual.k - data.k).abs <= 0.01*data.k) &&
        ((actual.uexp - data.uexp).abs <= 0.05*data.uexp)
      end
